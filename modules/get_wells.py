...
def get_wells(g):
    ...
    if (len(filtered) != g.rows*g.columns):
        print("The number of identified wells {} does not match expectation of {} from HTD file".format(len(filtered), g.rows*g.columns))
    if (time_points != g.time_points):
        print("The number of identified time points {} does not match expectation of {} from HTD file".format(time_points, g.time_points))
