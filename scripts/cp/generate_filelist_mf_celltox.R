...
work <- stringr::str_replace(input, "input", "work")
image_dir <- stringr::str_c(work, plate, sep = "/")
...
load_csv <- dplyr::tibble(
  ...
  URL_GFP = stringr::str_c("file:", substring(work, 2), plate, input_files, sep = "/"),
...
readr::write_csv(load_csv, file = stringr::str_c(input, "/image_paths_mf_celltox.csv", sep = ""))
