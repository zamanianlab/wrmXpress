...
load_csv <- dplyr::tibble(
  ...
  URL_GFP = stringr::str_c("file:", substring(input, 2), plate, gfp, sep = "/"),
  URL_TransmittedLight = stringr::str_c("file:", substring(input, 2), plate, tl, sep = "/"),
  URL_TxRed = stringr::str_c("file:", substring(input, 2), plate, txrd, sep = "/"),
  URL_wellmask = stringr::str_c("file:", "/tmp/wrmXpress", "cp_pipelines", "masks", mask, sep = "/"),
...
readr::write_csv(load_csv, file = stringr::str_c(input, "/image_paths_feeding.csv", sep = ""))
